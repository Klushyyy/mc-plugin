...
public class InfernoMobPhase extends PhaseBase implements Listener {
    ...
    private void applyModifiers(LivingEntity mob) {
        modifyAttr(mob, Attribute.MAX_HEALTH,     v -> v * 2.0);
        AttributeInstance hp = mob.getAttribute(Attribute.MAX_HEALTH);
        if (hp != null) mob.setHealth(hp.getValue());

        modifyAttr(mob, Attribute.MOVEMENT_SPEED, v -> v * 2.0);
        modifyAttr(mob, Attribute.ATTACK_DAMAGE,  v -> v * 2.0);
        modifyAttr(mob, Attribute.FOLLOW_RANGE,   v -> 80.0);
