...
public class InfernoMobPhase extends PhaseBase implements Listener {
    ...
    public void start() {
        ...
        later(20*300, () -> { broadcast("§4§l[Volcano] FINAL PUSH — one last wave!"); wave(12); });
    ...
    private void applyModifiers(LivingEntity mob) {
        modifyAttr(mob, Attribute.GENERIC_MAX_HEALTH,     v -> v * 2.0);
        AttributeInstance hp = mob.getAttribute(Attribute.GENERIC_MAX_HEALTH);
        if (hp != null) mob.setHealth(hp.getValue());

        modifyAttr(mob, Attribute.GENERIC_MOVEMENT_SPEED, v -> v * 2.0);
        modifyAttr(mob, Attribute.GENERIC_ATTACK_DAMAGE,  v -> v * 2.0);
        modifyAttr(mob, Attribute.GENERIC_FOLLOW_RANGE,   v -> 80.0);
    ...
    private void modifyAttr(LivingEntity mob, Attribute attr, DoubleUnaryOperator fn) {
